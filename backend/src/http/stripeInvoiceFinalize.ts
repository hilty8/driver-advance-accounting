...
export const createStripeInvoiceFinalizeHandler = () => {
  return async (input: HandlerInput): Promise<HandlerResponse> => {
    const parsed = FinalizeSchema.safeParse(input.body);
    if (!parsed.success) {
    ...
    const invoice = await stripe.invoices.finalizeInvoice(run.stripe_invoice_id, { auto_advance: true });

    const updated = await prisma.billing_runs.update({
      where: { id: run.id },
      data: { stripe_status: invoice.status ?? 'finalized' }
    });
